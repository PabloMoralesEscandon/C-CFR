...
static Status traverse_average_node(
    ...
    MccfrSequentialWorkspace *workspace, InfoNode *node, size_t action_count,
    Utility *utility_out) {
    MccfrSequentialFrame *frame = &workspace->frames[depth];
    if (actor == target_player) {
        const Status status = record_sampled_strategy(
            workspace, node, frame, action_count, strategy_weight);
        if (status != CFR_STATUS_SUCCESS)
            return status;
    }
...
static Status traverse_branch(const CfrTraversalAdapter *adapter,
    ...
    if (workspace->average_only) {
        return traverse_average_node(
            ...
            own_reach, workspace, node, action_count, utility_out);
    }
...
static Status configure_adapter(const Game *game, GameState *state,
    ...
    if (game == NULL || state == NULL || store == NULL || rng == NULL ||
        ...
        game->strategic_player_count == 0 || game->strategic_player_count > CFR_MAX_PLAYERS ||
        !cfr_traversal_player_is_valid(
