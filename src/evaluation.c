...
#define EVALUATION_ABS_EPSILON 1e-12
#define EVALUATION_INDEX_EMPTY SIZE_MAX
#define EVALUATION_REL_EPSILON 1e-8
#define INITIAL_EDGE_CAPACITY 8
#define INITIAL_FRAME_CAPACITY 8
#define INITIAL_GROUP_CAPACITY 8
#define INITIAL_GROUP_TABLE_CAPACITY 8
...
static Status workspace_init(EvaluationWorkspace *workspace,
    ...
    temporary.nodes = malloc(INITIAL_NODE_CAPACITY * sizeof(*temporary.nodes));
    if (temporary.nodes == NULL) {
        workspace_destroy(&temporary);
        return CFR_STATUS_OUT_OF_MEMORY;
    }

    temporary.edges = malloc(INITIAL_EDGE_CAPACITY * sizeof(*temporary.edges));
...
static Status ensure_frame(EvaluationWorkspace *workspace, size_t depth) {
    ...
    if (required_count > workspace->frame_capacity) {
        ...
        EvaluationFrame *grown =
            realloc(workspace->frames, new_capacity * sizeof(EvaluationFrame));
    ...
    while (workspace->frame_count < required_count) {
        Action *actions = malloc(workspace->max_legal_actions * sizeof(Action));
...
static Status workspace_append_node(EvaluationWorkspace *workspace,
    ...
    if (workspace->node_count == workspace->node_capacity) {
        ...
        EvaluationNode *grown =
            realloc(workspace->nodes, new_capacity * sizeof(*workspace->nodes));
        if (grown == NULL)
            return CFR_STATUS_OUT_OF_MEMORY;
...
static Status workspace_reserve_edges(EvaluationWorkspace *workspace,
    ...
    if (required_count > workspace->edge_capacity) {
        ...
        EvaluationEdge *grown =
            realloc(workspace->edges, new_capacity * sizeof(*workspace->edges));
        if (grown == NULL)
            return CFR_STATUS_OUT_OF_MEMORY;
