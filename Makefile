...
CPP_HEADER_TEST_SOURCE := tests/test_cpp_headers.cpp
TRAINING_BENCHMARK_SOURCE := benchmarks/training_benchmark.c
EVALUATION_BENCHMARK_SOURCE := benchmarks/evaluation_benchmark.c
CHECKPOINT_BENCHMARK_SOURCE := benchmarks/checkpoint_benchmark.c
...
CPP_HEADER_TEST_OBJECT := \
	$(patsubst %.cpp,$(DEBUG_DIR)/%.o,$(CPP_HEADER_TEST_SOURCE))
TRAINING_BENCHMARK_OBJECT := \
	$(patsubst %.c,$(RELEASE_DIR)/%.o,$(TRAINING_BENCHMARK_SOURCE))
EVALUATION_BENCHMARK_OBJECT := \
	$(patsubst %.c,$(RELEASE_DIR)/%.o,$(EVALUATION_BENCHMARK_SOURCE))
CHECKPOINT_BENCHMARK_OBJECT := \
	$(patsubst %.c,$(RELEASE_DIR)/%.o,$(CHECKPOINT_BENCHMARK_SOURCE))
...
TRAINING_BENCHMARK_BINARY := $(RELEASE_DIR)/training-benchmark
EVALUATION_BENCHMARK_BINARY := $(RELEASE_DIR)/evaluation-benchmark
CHECKPOINT_BENCHMARK_BINARY := $(RELEASE_DIR)/checkpoint-benchmark
...
DEPENDENCY_FILES := \
	...
	$(CPP_HEADER_TEST_OBJECT:.o=.d) \
	$(TRAINING_BENCHMARK_OBJECT:.o=.d) \
	$(EVALUATION_BENCHMARK_OBJECT:.o=.d) \
	$(CHECKPOINT_BENCHMARK_OBJECT:.o=.d) \
...
benchmark: $(TRAINING_BENCHMARK_BINARY) $(EVALUATION_BENCHMARK_BINARY) \
	$(CHECKPOINT_BENCHMARK_BINARY)
...
$(TEST_BINARY): $(TEST_OBJECTS) $(CPP_HEADER_TEST_OBJECT) $(DEBUG_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(TEST_OBJECTS) $(CPP_HEADER_TEST_OBJECT) \
		$(DEBUG_LIBRARY) $(LDLIBS) -o $@

$(CPP_HEADER_TEST_OBJECT): $(CPP_HEADER_TEST_SOURCE)
	@mkdir -p $(dir $@)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $(CXX_STANDARD) $(CXX_WARNINGS) \
		$(DEBUG_FLAGS) $(DEPENDENCY_FLAGS) -c $< -o $@

$(BLACKJACK_TEST_BINARY): $(BLACKJACK_TEST_OBJECT) $(DEBUG_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(BLACKJACK_TEST_OBJECT) $(DEBUG_LIBRARY) $(LDLIBS) -o $@
...
$(RELEASE_BINARY): $(RELEASE_APP_OBJECT) $(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(RELEASE_APP_OBJECT) \
		$(RELEASE_LIBRARY) $(LDLIBS) -o $@

$(DEBUG_BINARY): $(DEBUG_APP_OBJECT) $(DEBUG_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(DEBUG_APP_OBJECT) $(DEBUG_LIBRARY) $(LDLIBS) -o $@

$(RELEASE_LEDUC_BINARY): $(RELEASE_LEDUC_APP_OBJECT) $(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(RELEASE_LEDUC_APP_OBJECT) \
		$(RELEASE_LIBRARY) \
		$(LDLIBS) -o $@

$(DEBUG_LEDUC_BINARY): $(DEBUG_LEDUC_APP_OBJECT) $(DEBUG_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(DEBUG_LEDUC_APP_OBJECT) $(DEBUG_LIBRARY) $(LDLIBS) \
		-o $@

$(RELEASE_BLACKJACK_BINARY): $(RELEASE_BLACKJACK_APP_OBJECT) $(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(RELEASE_BLACKJACK_APP_OBJECT) \
		$(RELEASE_LIBRARY) \
		$(LDLIBS) -o $@
...
$(RELEASE_BLACKJACK_COMPACT_EVAL_BINARY): \
	...
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(RELEASE_BLACKJACK_COMPACT_EVAL_OBJECT) \
		$(RELEASE_LIBRARY) $(LDLIBS) -o $@

$(RELEASE_POKER_TREE_EXPORT_BINARY): $(RELEASE_POKER_TREE_EXPORT_OBJECT) \
		$(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(RELEASE_POKER_TREE_EXPORT_OBJECT) \
		$(RELEASE_LIBRARY) $(LDLIBS) -o $@

$(DEBUG_BLACKJACK_BINARY): $(DEBUG_BLACKJACK_APP_OBJECT) $(DEBUG_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(DEBUG_BLACKJACK_APP_OBJECT) $(DEBUG_LIBRARY) \
		$(LDLIBS) -o $@

$(TRAINING_BENCHMARK_BINARY): $(TRAINING_BENCHMARK_OBJECT) $(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(TRAINING_BENCHMARK_OBJECT) \
		$(RELEASE_LIBRARY) $(LDLIBS) -o $@

$(EVALUATION_BENCHMARK_BINARY): $(EVALUATION_BENCHMARK_OBJECT) \
		$(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(EVALUATION_BENCHMARK_OBJECT) \
		$(RELEASE_LIBRARY) $(LDLIBS) -o $@

$(CHECKPOINT_BENCHMARK_BINARY): $(CHECKPOINT_BENCHMARK_OBJECT) \
		$(RELEASE_LIBRARY)
	@mkdir -p $(dir $@)
	$(CXX) $(LDFLAGS) $(RELEASE_LINK_FLAGS) $(CHECKPOINT_BENCHMARK_OBJECT) \
		$(RELEASE_LIBRARY) $(LDLIBS) -o $@

# The implementation uses C++20 while applications and tests remain C17. This
# keeps the public ABI usable from C and verifies that boundary on every build.
$(RELEASE_OBJECTS): $(RELEASE_DIR)/%.o: %.c
	@mkdir -p $(dir $@)
	$(CXX) -x c++ $(CPPFLAGS) $(CFLAGS) $(CXXFLAGS) $(CXX_STANDARD) \
		$(CXX_WARNINGS) $(RELEASE_FLAGS) $(DEPENDENCY_FLAGS) -c $< -o $@

$(DEBUG_OBJECTS): $(DEBUG_DIR)/%.o: %.c
	@mkdir -p $(dir $@)
	$(CXX) -x c++ $(CPPFLAGS) $(CFLAGS) $(CXXFLAGS) $(CXX_STANDARD) \
		$(CXX_WARNINGS) $(DEBUG_FLAGS) $(DEPENDENCY_FLAGS) -c $< -o $@
