...
typedef struct {
    ...
    size_t fail_after;
    InfoNode *mutated_node;
    size_t *root_visits;
    bool change_action_order;
} MultiplayerContext;
...
static Status update_root_before_average(const void *raw_context,
                                         const GameState *raw_state,
                                         Actor *result) {
    const MultiplayerContext *context = raw_context;
    const MultiplayerState *state = (const MultiplayerState *)raw_state;
    if (state->depth == 0 && ++*context->root_visits == 2) {
        const Utility regrets[] = {-1.0, 1.0};
        const double strategy[] = {0.0, 0.0};
        const Status status = cfr_info_node_apply_deltas(
            context->mutated_node, regrets, strategy, 2);
        if (status != CFR_STATUS_SUCCESS)
            return status;
    }
    return current_actor(raw_context, raw_state, result);
}
...
static Status average_legal_actions(const void *raw_context,
                                    const GameState *raw_state, Action *actions,
                                    size_t capacity, size_t *count) {
    const MultiplayerContext *context = raw_context;
    const MultiplayerState *state = (const MultiplayerState *)raw_state;
    const Status status = legal_actions(raw_context, raw_state, actions,
                                        capacity, count);
    if (status == CFR_STATUS_SUCCESS && context->change_action_order &&
        state->depth == 0 && *context->root_visits == 2) {
        actions[0] = 1;
        actions[1] = 0;
    }
    return status;
...
/* The average pass must use the regret pass's snapshot even if another
 * worker publishes a new root strategy between the two passes. */
static void test_average_reuses_snapshot(bool change_action_order) {
    InfoStore store = {0};
    MultiplayerState state = {0};
    size_t root_visits = 0;
    CHECK(cfr_info_store_init(&store) == CFR_STATUS_SUCCESS);
    InfoNode *root = node_for(&store, 1);
    const Utility initial_regrets[] = {0.75, 0.25};
    const double initial_strategy[] = {0.0, 0.0};
    CHECK(cfr_info_node_apply_deltas(root, initial_regrets, initial_strategy,
                                     2) == CFR_STATUS_SUCCESS);
    MultiplayerContext context = {
        .actors = {0, 1, 2, 3}, .decisions = 4, .zero_utility = true,
        .mutated_node = root, .root_visits = &root_visits,
        .change_action_order = change_action_order};
    GameOperations callbacks = operations;
    callbacks.current_actor = update_root_before_average;
    callbacks.legal_actions = average_legal_actions;
    Game game = game_for(&context);
    game.operations = &callbacks;
    MccfrRng rng;
    ...
    const Status status = cfr_mccfr_external_traverse_with_stats(
        &game, public_state(&state), &store, CFR_PLAYER_0, &rng, &utility,
        &stats);
    CHECK(root_visits == 2);
    CHECK(state.depth == 0 && state.prefix == 0);
    CHECK(root->regret_sums[0] == -0.25 && root->regret_sums[1] == 1.25);
    if (change_action_order) {
        CHECK(status == CFR_STATUS_INVALID_ARGUMENT);
        CHECK(root->strategy_sums[0] == 0.0 && root->strategy_sums[1] == 0.0);
        CHECK(rng.state == before.state);
        CHECK(utility == 123.0 && stats.visited_nodes == 456);
    } else {
        CHECK(status == CFR_STATUS_SUCCESS);
        CHECK(root->strategy_sums[0] == 0.75 && root->strategy_sums[1] == 0.25);
        CHECK(utility == 0.0 && stats.visited_nodes == 14);
        CHECK(rng.state == before.state + 10 * UINT64_C(0x9e3779b97f4a7c15));
    }
...
int test_multiplayer(void) {
    failures = 0;
    test_average_reuses_snapshot(false);
    test_average_reuses_snapshot(true);
